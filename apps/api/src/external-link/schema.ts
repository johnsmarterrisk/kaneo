...
export const createExternalLinkBody = z.object({
  ...
  // "message" or "file", and a closed enum rather than a free string.
  //
  // This route is Telegraph's, and Telegraph's external resources are a Nostr
  // event rendered as a message and a Stash file. Two things follow. It is part
  // of migration 0045's unique key `(task_id, integration_id, resource_type,
  // external_id)`, so a free string would let one caller fragment its own
  // idempotency by varying it. And a caller able to name any resource type on a
